...
from ops.dust3r import Dust3r_Tool
...
class Pipeline():
    ...
    def init_scene(self):
        ...
        for i,dust3r_frame in enumerate(dust3r_frames):
            ...
            rgb = np.array(Image.open(self.files[i]))/255.
            rgb = rgb[:,:,0:3]
            ...
            # add
            frame = Frame(H=H,W=W,rgb=rgb,dpt=dpt,sky=sky,
    ...
    def _coarse_scene(self):
        ...
        for i in range(self.n_sample+2-len(self.files)):
            print(f'Procecssing {i+len(self.files)}/{self.n_sample} frame...')
