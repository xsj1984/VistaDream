...
class PairViewer (BasePCOptimizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(self.n_imgs):
            ...
            H, W = self.imshapes[i]
            pts3d = self.pred_i[edge_str(i, 1-i)].cpu()
            ...
            try:
                ...
                pose = np.eye(4)
            rel_poses.append(torch.from_numpy(pose.astype(np.float32)).to(self.pred_j['1_0']))

        # let's use the pair with the most confidence
        if confs[0] > confs[1]:
            # ptcloud is expressed in camera1
            self.im_poses = [torch.eye(4).to(rel_poses[1]), rel_poses[1]]  # I, cam2-to-cam1
            self.depth = [self.pred_i['0_1'][..., 2], geotrf(inv(rel_poses[1]), self.pred_j['0_1'])[..., 2]]
        else:
            # ptcloud is expressed in camera2
            self.im_poses = [rel_poses[0], torch.eye(4).to(rel_poses[0])]  # I, cam1-to-cam2
