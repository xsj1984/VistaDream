...
class Hack_worker():
    ...
    def handler(self, async_task):
        ...
        outpaint_extend_times = args.pop()
        ...
        if 'upscale' in goals:
            ...
            if direct_return:
                ...
                outputs.append(uov_input_image)
                return
        ...
        if 'inpaint' in goals:
            if len(outpaint_selections) > 0:
                ...
                if 'bottom' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[0, int(H * extend_times)], [0, 0], [0, 0]], mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[0, int(H * extend_times)], [0, 0]], mode='constant',
                                          constant_values=255)

                H, W, C = inpaint_image.shape
                if 'left' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[0, 0], [int(W * extend_times), 0], [0, 0]], mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[0, 0], [int(W * extend_times), 0]], mode='constant',
                                          constant_values=255)
                if 'right' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[0, 0], [0, int(W * extend_times)], [0, 0]], mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[0, 0], [0, int(W * extend_times)]], mode='constant',
                                          constant_values=255)
